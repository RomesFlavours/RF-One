...
**Also not part of the table above, listed here only for discoverability:** [15_Human_Operational_State_and_Adaptive_Cognito_Interaction.md](15_Human_Operational_State_and_Adaptive_Cognito_Interaction.md) — status "APPROVED — Post-Baseline Conceptual Extension," produced after `release/rf-one-2.0` (tag `rf-one-2.0-baseline`) and ratified by the Product Owner after that baseline. It defines Human Operational State (the temporary, contextual condition of a person at the current operational moment) and how Cognito's Human Interaction capability may adapt to it, without creating Authority, diagnosing a medical condition, or redefining Cognito, Attention Management or Organizational Responsibility. Ratification approves it as a stable, standalone post-baseline extension: it remains outside the canonical 00–14 set above and does not retroactively extend `core-2.0-freeze`.

**Also not part of the table above, listed here only for discoverability:** [16_Ambient_Operational_Context_and_In-Flow_Cognito_Assistance.md](16_Ambient_Operational_Context_and_In-Flow_Cognito_Assistance.md) — status "APPROVED — Post-Baseline Conceptual Extension," ratified by the Product Owner after document 15 above, on a further dedicated branch. It defines Ambient Operational Context (what is happening in the work around a person, distinct from Human Operational State) and In-Flow Cognito Assistance (a specialization of Cognito's Human Interaction capability that uses it), without creating Authority, without authorizing surveillance or permanent recording, and without redefining Cognito, Attention Management, Trigger Intelligence, or Organizational Responsibility. It recognizes the Restaurant Domain's existing Service Copilot module as its first Domain-level consumer instance, without redefining, renaming, or merging it with Cognito. Ratification approves it as a stable, standalone post-baseline extension: it remains outside the canonical 00–14 set above and does not retroactively extend `core-2.0-freeze`.

**Also not part of the table above, listed here only for discoverability:** [17_Visual_Video_Context_as_Ambient_Evidence.md](17_Visual_Video_Context_as_Ambient_Evidence.md) — status "PROPOSED — Post-Baseline Conceptual Extension," a sub-extension of document 16 above, produced on the same dedicated branch. It documents Visual/Video Context as one source category of Ambient Operational Context — authorized images/video as scene-level Ambient Evidence, never automatic Fact, never a judgment about a person, never a basis for face/biometric/emotion recognition — without creating Authority, without authorizing surveillance, and without redefining Ambient Operational Context, Cognito, Human Operational State, Attention Management, or Trigger Intelligence. It remains outside the canonical 00–14 set above and does not extend `core-2.0-freeze`.
