...
**Also not part of the table above, listed here only for discoverability:** [15_Human_Operational_State_and_Adaptive_Cognito_Interaction.md](15_Human_Operational_State_and_Adaptive_Cognito_Interaction.md) — status "PROPOSED — Post-Baseline Conceptual Extension," produced after `release/rf-one-2.0` (tag `rf-one-2.0-baseline`). It defines Human Operational State (the temporary, contextual condition of a person at the current operational moment) and how Cognito's Human Interaction capability may adapt to it, without creating Authority, diagnosing a medical condition, or redefining Cognito, Attention Management or Organizational Responsibility. It awaits Product Owner ratification before being folded into the canonical set above.
