...
- **`Shared Domains/`** — Domains whose concepts are genuinely industry-independent and must remain reusable by any business, not just Restaurant. A Shared Domain may consume industry-specific content a Business Domain supplies, but must not structurally depend on one.
...
| `_Shared/` | *(not a Domain)* | Domain-independent-but-not-universal shared knowledge reused across multiple Domains (currently `Environment/`). Deliberately outside both `Shared Domains/` and `Business Domain/` — see "Shared Domains vs. Business Domain" above. |
