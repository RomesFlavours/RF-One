...
Personnel Management (`01 Domains/Cross Domain/Personnel Management/README.md`) may consume `Total Employee Cost` as evidence — for example when a Personnel Decision compares the current person's expected value against an alternative's. Personnel Cost supplies facts; Personnel Management owns the decision.
