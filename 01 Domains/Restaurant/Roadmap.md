...
- **Workforce / Personnel** — employees, roles, scheduling, payroll, skills, performance. Currently only referenced in passing (`Model/OperationalArea.md`, "Employees are assigned to Operational Areas"). Approved sequencing: when People/Workforce modeling becomes necessary, establish reusable Workforce semantics (role, assignment, responsibility, schedule, skills/capabilities, availability, performance-related facts) first; only then design Selection, Training, and Performance capabilities on top of them. Approved future direction: `Goals → Brand → Service Model → Behaviors → Selection / Training / Performance`. No Domain or Product capability is created now; no final name (`Workforce`, `People`, `Personnel`, `HR`) is chosen yet.
