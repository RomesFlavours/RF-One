...
**Training is the internal RF-One service responsible for the entire training cycle.** It is described functionally, the same way Selection already is — not as a new top-level Shared Domain, not as a Product, and not owned by Operational Knowledge or by Continuous Productivity Development.
...
**This does not move, rename, or delete the `01 Domains/Shared Domains/Operational Knowledge/` folder.** The current document organization — Operational Knowledge as its own folder inside `01 Domains/Shared Domains/` — is unchanged by this decision. See §6 for why the functional decision and the folder's physical location are deliberately kept distinct here.
