...
### 20.2 Account birth and the validated data horizon (BANK_ACCOUNT_BIRTH_AND_VALIDATED_HORIZON_001)

- **Birth is derived, death is decided.** `effective_start_date` = the instrument's first eligible posting date (`monthly_source.first_posting_date`, the same population as the closure rule: confirmed or suppressed duplicates excluded). It is filled when empty, or moved earlier when a real transaction proves an earlier life; never invented when no posting date exists. Months before birth are NOT_EXPECTED through the existing `evaluate_expectation`. The LAST transaction never ends a life: an instrument that goes quiet stays ACTIVE with no end date until a human resolves the month — `NO_ACTIVITY` ("existed, stayed active, nothing happened") or a lifecycle-ending resolution, which derives `effective_end_date` from MAX(eligible posting date) as before.
- **Validated through.** `bank_reconciliation_control_configs.validated_through_month` (`YYYY-MM`, migration `d3f8a61c2e47`) is the last month whose loaded data a human has certified complete. The enforced interval is control start → validated through. A later month is **provisional**: its data and period are kept, it is not opened or refreshed automatically, its gaps are shown as information but never enforced, it cannot be certified COMPLETE and it proves nothing about missing files or an account's life. Advancing the horizon brings those months under the ordinary rules from the batches already imported; moving it back deletes nothing. NULL = no horizon (previous behaviour).
- Within the validated horizon, an expected instrument with no source is asked neutrally: still active with no activity, closed, or source file missing.
- **STILL ACTIVE** (`RESOLUTION_STILL_ACTIVE`, BANK_EXPLICIT_STILL_ACTIVE_DECISION_001, migration `e7b2c94d0f18`) is the human's explicit confirmation that an INACTIVE instrument is alive. It resolves the month and sets the instrument ACTIVE; it writes no start or end date, implies no source or transaction and resolves no other month. It is accepted only for an INACTIVE instrument with no end date, no lifecycle reason and no lifecycle-ending resolution in any month (`monthly_source.still_active_refusal`); it never reopens a closed instrument. For an instrument that is already ACTIVE, `NO_ACTIVITY` remains the unchanged answer. Clearing the resolution does not reverse the status, as for every lifecycle change.
