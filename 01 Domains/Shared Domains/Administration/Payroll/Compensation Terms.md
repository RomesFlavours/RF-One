...
A compensation change creates a **new** valid interval; the prior row's `valid_to` is closed, never rewritten in place. This is required to reconstruct historical payroll in case of dispute — mirroring the existing `EmployeeAssignment` temporal pattern (`01 Domains/Business Domain/Restaurant/Organization/Employee Assignment.md`).
