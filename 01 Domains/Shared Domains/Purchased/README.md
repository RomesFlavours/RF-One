...
**Module:** Shared Domain / Purchased
...
RF-One already has a pre-existing **Invoice Intake** capability (`../Administration/Invoice Intake/README.md`), documented as "a Shared Domain capability... document acquisition, OCR/parsing, normalization, review, and routing." As of "Align legacy Invoice Intake with Purchased" (see "Relationship to Restaurant's existing Purchasing module" below, "Invoice Intake alignment (closed)"), its legacy runtime prototype (`03 Software/InvoiceIntake/`) feeds Purchased's persistence directly — Restaurant/Purchasing consumes that output rather than owning it.
