...
No role name here is universal — a Restaurant configures whichever `RestaurantRole` values it actually uses (see `01 Domains/Business Domain/Restaurant/Organization/Restaurant Role.md`).
...
Concurrent Employee Assignments may be legitimate (`01 Domains/Business Domain/Restaurant/Restaurant Semantic Model.md` §9; `01 Domains/Business Domain/Restaurant/Organization/Employee Assignment.md`, "Multi-role / multi-area capability" — e.g. an Employee holding both a `Manager` and a `Server` Assignment at once, in the same or different Operational Areas). Tip eligibility is evaluated against the specific policy component being calculated — the presence of another, concurrent, matching-a-different-Role Assignment does not by itself invalidate eligibility for this component.
...
- `ROLE_PRESENT_AT_PAYMENT` never silently reduces to a Clover `SourceRole` — see `01 Domains/Business Domain/Restaurant/Organization/Restaurant Role.md`, "Distinct from Clover named Role, Clover systemRole, and personnel identity."
