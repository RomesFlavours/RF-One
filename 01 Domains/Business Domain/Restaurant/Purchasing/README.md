...
See also `01 Domains/Cross Domain/Purchased/README.md` — a later, cross-Business-Domain Shared Domain formalizing the general "purchase fact" concept (what was invoiced, one normalized line per economic item). It does not rename, restructure, or take over this module's Purchase Document/Purchase Line/Effective Product Cost *model*, which remains exactly as approved here — the underlying `PurchaseDocument`/`PurchaseLine` schema is unchanged. What has changed, for the `03 Software/InvoiceIntake/` path specifically (Purchased README, "Invoice Intake alignment (closed)"): **capture-and-normalize ownership** of that Purchase Fact now belongs to Purchased, not to this module — this module is a *consumer* of it for Purchase Order, Configured Expectation, Physical Receiving, Reconciliation, Alert and Expected Supplier Credit. Whether this module's remaining scope should itself eventually be re-expressed as a Purchased consumer/specialization more broadly stays an open question (see also `01 Domains/Domain Architecture.md` §9, Open Question 6).
