...
Invoice Intake — reading a supplier's invoice, receipt, credit note or API purchase record and turning it into a Purchase Document — was originally documented as a capability of this module. As of "Align legacy Invoice Intake with Purchased," the capture-and-normalize act itself (OCR/parsing, human review, producing the Purchase Document/Purchase Line) is owned by **Purchased** (`01 Domains/Cross Domain/Purchased/README.md`, a Cross Domain) for the `03 Software/InvoiceIntake/` runtime path — see that README's "Invoice Intake alignment (closed)." This module consumes the resulting Purchase Document; it does not need to own the intake step to do so. This applies equally to mixed-type suppliers (e.g. broadline foodservice distributors selling Food, Drink and Supplies on the same invoice, such as Ben E. Keith, Cheney Brothers and Gordon Food): every purchased line is preserved as a Purchase Line, classified via Merchandise/Economic Classification, and costed via Effective Product Cost — see `EntityDefinitions.md` and `BusinessRules.md`. See `DataAcquisition.md` for the supported acquisition channels.
...
See also `01 Domains/Cross Domain/Purchased/README.md` — a later, cross-Business-Domain Cross Domain formalizing the general "purchase fact" concept (what was invoiced, one normalized line per economic item). It does not rename, restructure, or take over this module's Purchase Document/Purchase Line/Effective Product Cost *model*, which remains exactly as approved here — the underlying `PurchaseDocument`/`PurchaseLine` schema is unchanged. What has changed, for the `03 Software/InvoiceIntake/` path specifically (Purchased README, "Invoice Intake alignment (closed)"): **capture-and-normalize ownership** of that Purchase Fact now belongs to Purchased, not to this module — this module is a *consumer* of it for Purchase Order, Configured Expectation, Physical Receiving, Reconciliation, Alert and Expected Supplier Credit. Whether this module's remaining scope should itself eventually be re-expressed as a Purchased consumer/specialization more broadly stays an open question (see also `01 Domains/Domain Architecture.md` §9, Open Question 6).
