...
- [../Domain Architecture.md](../../Domain%20Architecture.md) — cross-Domain conclusions on Restaurant's boundary, the Shared Domains / Business Domain taxonomy (§4: Selection, Operational Knowledge (formerly Training), Continuous Productivity Development, Performance, Personnel Management, Taxation, Administration are Shared Domainss; Restaurant is the Business Domain), and the remaining transversal Domain candidates (Customer Feedback, Review)
...
- **Commercial Catalog** — stays under `01 Domains/Business Domain/Restaurant/Commercial Catalog/` now. It is the **highest-confidence future Shared Domain extraction candidate**: all 17 files are self-declared industry-independent with zero Restaurant-specific fields. Approved trigger: *extract to `01 Domains/_Shared/Commercial Catalog/` when a second genuine Domain or Product requires the same catalog semantics.* Do not split the folder concept-by-concept; if extraction occurs, the whole coherent model moves together unless new evidence creates a natural seam.
...
- **Workforce / Personnel** — employees, roles, scheduling, payroll, skills, performance. Currently only referenced in passing (`Model/OperationalArea.md`, "Employees are assigned to Operational Areas"). This area is now split between the **Personnel Management** transversal Shared Domains (`01 Domains/Shared Domains/Personnel Management/`, created by TASK_DOMAINS_002), whose remaining modules are Workforce and Personnel Decisions (both minimal placeholder READMEs — establishing Workforce semantics (role, assignment, responsibility, schedule, skills/capabilities, availability) in depth remains the previously recorded sequencing preference before Continuous Productivity Development is modeled further), and sibling top-level Shared Domainss extracted from it: **Selection** (`01 Domains/Shared Domains/Selection/`, TASK_DOMAINS_003), **Continuous Productivity Development** (`01 Domains/Shared Domains/Continuous Productivity Development/`, draft — conceptually superseded the former Training placeholder), **Operational Knowledge** (`01 Domains/Shared Domains/Operational Knowledge/`, the former Training folder, redefined as the shared retrievable-information repository), and **Performance** (`01 Domains/Shared Domains/Performance/`, documented in depth — TASK_PERSONNEL_001) — all extracted from Personnel Management, not modules of it, per the Shared Domains / Business Domain reorganization. Selection's Restaurant Industry Extension lives at `Restaurant/Selection/`. Approved direction: `Goals → Brand → Service Model → Behaviors → Selection / Continuous Productivity Development / Operational Knowledge / Performance / Personnel Management (Workforce / Personnel Decisions)`. See [../Domain Architecture.md](../../Domain%20Architecture.md) §4, [../Selection/README.md](../../Shared%20Domains/Selection/README.md), [../Continuous Productivity Development/README.md](../../Shared%20Domains/Continuous%20Productivity%20Development/README.md), [../Operational Knowledge/README.md](../../Shared%20Domains/Operational%20Knowledge/README.md), [../Performance/README.md](../../Shared%20Domains/Performance/README.md) and [../Personnel Management/README.md](../../Shared%20Domains/Personnel%20Management/README.md) for how Selection, Workforce, Continuous Productivity Development, Operational Knowledge, Performance and Personnel Decisions are currently distinguished from one another. **Update (TASK_SERVER_PERFORMANCE_001):** "No Restaurant Product capability is created now" no longer holds for the Server role specifically — `Server Performance/`, `Service Copilot/` and `Dining Intelligence/` are now Restaurant-owned technical specializations that consume, and supply technical content to, the sibling Performance Shared Domains without redefining it (`Server Performance/README.md`, "Naming and boundary"). This does not create a Workforce, generic HR, Payroll, or table-assignment capability — see `Server Performance/Exclusions.md`.
...
6. Validate Sales / Organization / Tips / Compensation / Payroll / Performance behavior end-to-end against real Mount Dora production data once ingested (not synthetic fixtures) before treating Mount Dora as production-ready.
