...
`deliver_to_invoice_intake()` calls the existing `ocr_engine.py` unchanged — it needs **Tesseract OCR** and **Poppler** actually installed to read scanned PDFs and photographed JPG/PNG attachments; a digital-text PDF never needs either (`ocr_engine.py` always tries the embedded-text path first — see `03 Software/InvoiceIntake/README.md`, "PDF digitale vs PDF scansionato"). Without them, every non-digital document acquired here is read as `"(Nessun motore disponibile per leggere questo PDF)"` (or nothing at all for an image) and — since that is indistinguishable from a genuinely empty/failed read — ends up HUMAN regardless of how legible the real invoice actually is. See `03 Software/InvoiceIntake/README.md`, "Requisiti," for install instructions (winget on Windows). **Supplier training on this pipeline should not begin until these are confirmed installed in the real deployment environment.**

## Not in scope for this task

OCR/parser *quality* is unchanged (`ocr_engine.py`/`parser.py` themselves are untouched) — this package is acquisition (and, as of "Enable real OCR and filter email signature assets," the runtime OCR environment) only.
