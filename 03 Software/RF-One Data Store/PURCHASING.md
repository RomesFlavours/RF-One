...
- **Human Review corrections are not (yet) visible to other Purchased/Purchasing consumers** — a correction lives in `purchased_field_corrections` and is merged in only by `human_review.effective_document_view()`; any OTHER reader of `PurchaseDocument`/`PurchaseLine` (Restaurant/Purchasing's own reconciliation, cost allocation, etc.) still sees the original, uncorrected columns. Wiring those consumers to merge corrections too is a deliberate follow-up, not done here (would touch code outside this task's scope).
