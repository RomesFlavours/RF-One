...
(TASK_TIPS_COMPLETE_001 §3/§16; STEP 12B integration) — Calculation
Schedule, Payment Schedule, and the two separate automatic scheduler loops.
Also covers the residual items not already exercised by `tips_payment_
execution_validation.py`: already-paid Tip Entitlements excluded from a
later Payment Cycle, and a Mercury `blocked` transaction status routed to
Attention.

Mirrors the existing Tips validation modules' pattern: synthetic fixture,
disposable database, a small fake Mercury client (wrapped in the real
`payment_connector.MercuryPaymentConnector` adapter — STEP 12B), always
rolled back. `tips.scheduler`'s connector-neutral AUTO WITHOUT APPROVAL
tick (`run_due_payment_cycle_auto_approvals`) is exercised separately in
`tips_payment_connector_validation.py`; this file only covers the
connector-agnostic halves of the scheduler (`run_due_calculations`,
`run_due_payment_cycle_starts`) plus direct `approve_and_pay_cycle` calls."""
...
class _FakeMercuryClient:
    ...
    `blocked` terminal transaction status. Wrapped in `payment_connector.
...
def _test_schedule_effective_dating(session: Session, result: ValidationResult) -> None:
    ...
    # and (STEP 12B) also carries the connector selection.
    pay_cfg = sched_svc.set_payment_schedule(
        ...
        connector_code="MERCURY", mercury_source_account_id="acct-configured", effective_from=t1, created_by="tester",
    )
    session.commit()
    result.check(
        "Payment Schedule is independent of Calculation Schedule and carries its own connector + Mercury "
        "source account",
        pay_cfg.connector_code == "MERCURY" and pay_cfg.mercury_source_account_id == "acct-configured"
        and pay_cfg.interval_days == 7,
    )

    invalid_connector_rejected = False
    try:
        sched_svc.set_payment_schedule(
            session, restaurant_id=restaurant.id, mode="MANUAL", connector_code="NOT_A_REAL_CONNECTOR",
        )
    except sched_svc.ScheduleConfigError:
        invalid_connector_rejected = True
    result.check(
        "validation: an unregistered connector_code is rejected at configuration time, not just at execution time",
        invalid_connector_rejected,
    )
...
def _test_already_paid_entitlements_excluded(session: Session, result: ValidationResult) -> None:
    ...
    session.add(
        m.EmployeeExternalPaymentAccount(employee_id=employee.id, provider="MERCURY", provider_recipient_id="recipient-paidexcl", is_active=True)
    )
    session.commit()
    ...
    cycle_svc.approve_and_pay_cycle(session, cycle=cycle1, acting_identity=approver, connector=connector, source_account_id="acct-1")
    session.commit()
...
def _test_blocked_mercury_status_raises_attention(session: Session, result: ValidationResult) -> None:
    ...
    cycle_svc.approve_and_pay_cycle(session, cycle=cycle, acting_identity=approver, connector=connector, source_account_id="acct-1")
    session.commit()
    ...
    result.check(
        "'blocked' status is a distinct, real exception path this test exercises",
    ...
    if instruction.status != "NEEDS_ATTENTION" and instruction.provider_transaction_id is not None:
        ...
        session.commit()
    result.check(
        "'blocked' status routes this instruction to NEEDS_ATTENTION with an AttentionItem raised",
