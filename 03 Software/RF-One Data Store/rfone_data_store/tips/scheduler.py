...
scheduler separati"; STEP 12B integration). Mirrors the existing
`technical.connectors.clover.live_sync._run_forever` shape (a testable
`run_due_*` function wrapped by a thin, uncovered `_run_forever`/`main`
process loop) — no new scheduling primitive, no cron/celery dependency
introduced.
...
already been handled ("NON hardcodare"). A Restaurant with no AUTOMATIC
configuration is simply never triggered here — it relies entirely on manual
"Run Calculation Now"/"Start Payment Cycle Now".

Calculation due-ness never itself bypasses `payout_process.run_calculation_
now`'s own Clover-readiness/already-calculated checks — this module only
decides WHEN to ATTEMPT, never whether the attempt succeeds.
...
Connector-neutral by construction (STEP 12B Product Owner decision): the
Payment loop never imports or constructs a Mercury client itself. Each
Restaurant's connector is resolved from ITS OWN configured `connector_code`
(`tips/payment_connector.resolve_connector`) inside the per-Restaurant loop
— never one client shared for the whole tick — so two Restaurants may in
principle use two different connectors in the same tick, and a Restaurant
with no/unknown connector configured fails closed for that Restaurant alone
(reported in its own outcome), never silently defaulted to Mercury and
never blocking any other Restaurant's tick."""
...
def run_due_payment_cycle_starts(session: Session, *, now: datetime | None = None) -> list[DuePaymentCycleOutcome]:
    ...
    Approves & Pays it (the Authority gate is never bypassed by automation;
    Approve & Pay always requires an explicit, authorized human action, even
    when the Cycle itself opened automatically). No connector is touched by
    this half of the tick at all."""
    now = now or datetime.now(UTC)
...
def run_due_payment_cycle_auto_approvals(
    ...
    connector_override: "connector_svc.PaymentConnector | None" = None,
    ...
    error here; a persistently-failing readiness raises Attention via the
    SAME mechanism the Web Payment Control route uses, never a
    Tips-specific one.

    `connector_override` — test-only: bypasses per-Restaurant connector
    resolution entirely (e.g. inject one fake connector for every
    Restaurant in a test tick). Production callers omit it: each
    Restaurant's connector is resolved from ITS OWN configured
    `connector_code` (STEP 12B) — a Restaurant with no/unknown connector
    configured fails closed for that Restaurant alone, reported in its own
    outcome, never silently defaulted to Mercury and never blocking any
    other Restaurant's tick."""
    now = now or datetime.now(UTC)
    ...
    for restaurant_id in _all_restaurant_ids(session):
        ...
        try:
            connector = connector_override or connector_svc.resolve_connector(config.connector_code)
        except connector_svc.ConnectorConfigurationError as exc:
            outcomes.append(
                DueAutoApprovalOutcome(
                    restaurant_id=restaurant_id, cycle_id=cycle.id, approved=False,
                    reason=f"payment connector configuration error: {exc}",
                )
            )
            continue

        try:
            source_account_id = cycle_svc.resolve_source_account_id(session, restaurant_id, connector)
            if source_account_id is None:
                outcomes.append(
                    DueAutoApprovalOutcome(
                        restaurant_id=restaurant_id, cycle_id=cycle.id, approved=False,
                        reason=f"no {connector.connector_code} source account configured or available",
                    )
                )
                continue
            ...
            cycle_svc.approve_and_pay_cycle(
                session, cycle=cycle, acting_identity=system_identity, connector=connector,
                source_account_id=source_account_id, now=now,
            )
            session.commit()
            ...
            outcomes.append(
                DueAutoApprovalOutcome(
                    restaurant_id=restaurant_id, cycle_id=cycle.id, approved=False,
                    reason=f"{connector.connector_code} connector error: {exc}",
                )
            )
    return outcomes


def run_payment_tick(
    session: Session, *, now: datetime | None = None,
    connector_override: "connector_svc.PaymentConnector | None" = None,
) -> list:
    ...
    approval_outcomes = run_due_payment_cycle_auto_approvals(session, now=now, connector_override=connector_override)
