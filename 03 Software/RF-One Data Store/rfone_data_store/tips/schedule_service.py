...
(TASK_TIPS_COMPLETE_001 §2/§3; STEP 12B integration).
...
Config` (WHEN to pay out, and WHICH payment connector to use). Neither
schedule is inferred from the other — a Restaurant may calculate daily
while paying out weekly, or any other combination. Mirrors the existing
effective-dating discipline already established by `distribution_rule_
service.create_new_version` and `payroll/payment_execution.py`'s
`approved_provider_at`: never overwrite a prior configuration row, close
its `valid_to` and insert a new one; resolve "what applies right now" by
picking the most-recently-started row whose `[valid_from, valid_to)` window
contains the instant in question."""
...
def set_calculation_schedule(
    ...
    inserts a new one — never overwrites history."""
...
def set_payment_schedule(
    session: Session, *, restaurant_id: int, mode: str, interval_days: int | None = None,
    execution_time: time | None = None, anchor_date: date | None = None,
    connector_code: str | None = None, mercury_source_account_id: str | None = None,
    auto_approval_mode: str | None = None, effective_from: datetime | None = None, created_by: str | None = None,
) -> m.TipsPaymentScheduleConfig:
    """`auto_approval_mode` (the three decided Tips payment modes) is only
    meaningful when `mode=AUTOMATIC`: `None`/`WITH_APPROVAL` means a human
    still Approves & Pays every cycle this configuration opens
    (pre-existing, unchanged behavior); `WITHOUT_APPROVAL` means the SYSTEM
    Acting Identity does, under whatever Delegated Authority
    (`AuthorityGrant`) this Restaurant has explicitly granted it — never a
    bypass of that gate. Silently ignored (never persisted) when
    `mode=MANUAL`, mirroring how `interval_days` is already ignored for
    MANUAL — approval mode has no meaning without automation.

    `connector_code` (STEP 12B — Product Owner decision: payment mode
    configuration identifies the connector to invoke) is validated against
    `payment_connector.KNOWN_CONNECTOR_CODES` — the same registry
    `resolve_connector` consults at execution time — so a caller cannot
    persist an unknown connector code; a caller may still leave it `None`
    ("not yet configured"), which execution treats as fail-closed, never a
    silent Mercury default. `mercury_source_account_id` remains
    Mercury-specific configuration data (see `models.
    TipsPaymentScheduleConfig` docstring), accepted regardless of
    `connector_code` but only ever consulted by the resolver when the
    resolved connector is Mercury."""
    _validate(mode=mode, interval_days=interval_days)
    ...
    known_codes = connector_svc.known_connector_codes()
    if connector_code is not None and connector_code not in known_codes:
        raise ScheduleConfigError(
            f"Invalid connector_code {connector_code!r} — must be one of {known_codes} or None."
        )
    effective_from = effective_from or datetime.now(UTC)
    ...
    config = m.TipsPaymentScheduleConfig(
        restaurant_id=restaurant_id, mode=mode, interval_days=interval_days, execution_time=execution_time,
        anchor_date=anchor_date, connector_code=connector_code,
...
def is_due(
    *, mode: str, interval_days: int | None, execution_time: time | None, anchor_date: date | None,
    ...
    when unset — a config that never specified one still behaves
    deterministically, anchored to whenever it was first evaluated due, not
    to a fabricated "day one" of the whole system.
