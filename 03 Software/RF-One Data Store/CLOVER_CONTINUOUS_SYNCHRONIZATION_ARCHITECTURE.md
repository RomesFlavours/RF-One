...
**Status:** APPROVED ARCHITECTURAL DECISION — the Correction/Reconciliation Poller §3 approves is now IMPLEMENTED (TASK_TIPS_RECONCILIATION_AND_PAYMENT_CONTROL_001, `technical/connectors/clover/reconciliation_poller.py`), per-Location granularity as §2's own Live Cursor already established; §3/§11 below are updated inline to record this, the rest of this document is otherwise unchanged from its original approval.
**Module:** Software / RF-One Data Store / Technical Connectors (Clover)
**Scope:** Originally a documentation-only architectural decision (no software/database/connector behavior changed by the decision itself); the Poller it approved has since been implemented as a separate, later change, noted inline where relevant rather than rewritten as if always true.
...
It formalizes a decision, not a redesign: Live Sync (already implemented) is confirmed as the correct shape for near-real-time acquisition; a Correction/Reconciliation Poller — approved here as the necessary complement to it, closing a real, verified gap (§3), and since implemented by TASK_TIPS_RECONCILIATION_AND_PAYMENT_CONTROL_001. No code, schema, or connector behavior changed as a result of THIS document at the time of its own approval; the later implementation task is recorded inline where relevant, not retroactively folded into this document's own original scope.
...
**Correction/Reconciliation Poller** is a second, distinct, continuous process — **implemented** (TASK_TIPS_RECONCILIATION_AND_PAYMENT_CONTROL_001, `technical/connectors/clover/reconciliation_poller.py`) — approved by this decision as a required complement to the Fast Live Extractor, not an alternative to it.
...
- **Relationship to existing `reconciliation.py`:** the existing module performs a one-time, post-bulk-ingestion validation (counts/monetary/empirical checks) against a completed historical load. The Correction/Reconciliation Poller is conceptually related (both compare source to canonical) but operationally distinct: it runs continuously against live, already-canonical data, looking for **changed** records, not validating a completed batch. **Resolved:** implemented as a new module, `technical/connectors/clover/reconciliation_poller.py`, mirroring `live_sync.py`'s own `compute_next_*_window`/`run_*_cycle`/`_run_forever`/`main` shape rather than extending `live_sync.py` itself or `freshness.py`'s on-demand pattern — reuses `acquisition.import_clover_period()` unchanged, adding `mode=MODE_RECONCILIATION` (filters Payments/Orders by `modifiedTime` instead of `createdTime`; also queries Orders directly by `modifiedTime` to catch an Order-only correction with no corresponding Payment change).
...
→ marks the last verified modifiedTime (or equivalent) checked for
  corrections, per Location
→ implemented: IngestionRun.mode = 'RECONCILIATION', per Clover-sourced
  Location (per-Location granularity — the same granularity choice §2
  already made and accepted for the Live Cursor; per-resource/table
  granularity remains a legitimate future refinement, not required)
```

**What already existed toward a Modification Cursor, and what closed the gap.** Clover's `modifiedTime` was already captured as raw evidence on ingestion — `mapping.py` maps it into `source_modified_at` (Item, Employee, and other catalog/reference entities) or `modified_at` (Order, Payment) — but no process read these columns to decide "what changed since I last checked." TASK_TIPS_RECONCILIATION_AND_PAYMENT_CONTROL_001 closed this: `IngestionRun` gained a real, queryable `mode` column (`BACKFILL`/`LIVE_SYNC`/`RECONCILIATION` — previously encoded only in free-text `notes`), and `reconciliation_poller.compute_next_reconciliation_window` reads the latest COMPLETE/PARTIAL run with `mode='RECONCILIATION'` for a Location as its own Modification Cursor checkpoint — a SEPARATE lineage from the Live Cursor (`live_sync.compute_next_sync_window`, which reads the latest run regardless of mode), so a Live Sync/Backfill run advancing far ahead never causes the Reconciliation Poller to believe it has already checked that far for corrections, and vice versa.
...
```text
THIS document itself (at its own original approval) changed any software,
  schema, migration, or connector behavior — the Poller it approved was
  implemented later, by TASK_TIPS_RECONCILIATION_AND_PAYMENT_CONTROL_001,
  and is noted inline above where the implementation resolved an item this
...
the Modification Cursor (§4) was implemented, persisted, or scheduled BY
  THIS document — it approved the target shape; TASK_TIPS_RECONCILIATION_
  AND_PAYMENT_CONTROL_001 shipped it later, per-Location, mirroring §2's own
  Live Cursor granularity (see §4's updated text above)
per-resource/table Modification Cursor or Live Cursor granularity is
  implemented — both remain per-Location, as accepted in §2/§4 above; a
  future per-resource/table refinement remains legitimate future work
