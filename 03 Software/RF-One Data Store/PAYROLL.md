...
`rfone_data_store/payroll/schedule.py` holds the code governing `PayrollSchedule`/`PayrollPeriod` (this package's own administrative cadence configuration), and it deliberately does **not** define any overtime computation. `PayrollSchedule.schedule_type` supports `WEEKLY`/`BIWEEKLY`/`MONTHLY` (`validate_schedule_type`). `WorkweekDefinition` is a separate table (`restaurant_id`, `start_weekday`, `valid_from`/`valid_to`) — never derived from `PayrollSchedule`. **Ownership note (Product Owner decision):** although `WorkweekDefinition`'s table is unchanged, the Workweek boundary it defines is conceptually owned by Compensation / Rule Matrix, not by this Administration/Payroll package, because it preserves factual/regulatory context relevant to Compensation and to the Payroll Provider handoff. RF-One does not contain, and does not plan, an Overtime Evaluator that calculates statutory Overtime — that statutory treatment is applied by the external Payroll Provider, downstream of the Payroll Handoff Connector. `WorkweekDefinition` remains useful here only as Rule Matrix / Compensation context and metadata: it bounds the Workweek so that RF-One can preserve and supply correct worked-time facts, never to compute an overtime result itself. Its pure calendar helper, `workweeks_within_period(period_start, period_end, start_weekday)`, has moved accordingly to `rfone_data_store/payroll_calculation/workweek.py` — nothing calls it to compute overtime; it exists so that the Payroll Provider has a Workweek boundary to consume. `PayrollRun.period_start`/`period_end`/`pay_date` are three independent columns; nothing derives one from another.
