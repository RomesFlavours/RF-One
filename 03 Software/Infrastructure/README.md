...
| Compensation — pagina provvisoria | `https://vhmsm9mgh8.us-east-1.awsapprunner.com/compensation` | 2026-09-11 | Stesso servizio `rfone-web`. Nessuna applicazione web Compensation esiste (solo codice di dominio/backend: `payroll_calculation/`, `compensation_approval_validation.py`) — pagina "Compensation — Work in progress" dietro `require_domain_access("COMPENSATION")`. `GET /compensation` senza login → 302 a `/login?next=/compensation` (verificato via curl, pubblico). Con account tecnico + accesso COMPENSATION su DB temporaneo locale → 200 con testo "Compensation — Work in progress" (verificato in locale, non in produzione). |

**Da non presentare come verificato senza ripetere il controllo:** questa tabella riflette lo stato osservato il 2026-09-11. Un nuovo deploy, un redeploy con nuovo URL, o l'integrazione futura di Selection/Compensation in AWS possono invalidarla — ripetere la ricognizione prima di riusarla come fonte. Le righe Selection/Compensation restano valide solo finché quelle route continuano a servire la pagina provvisoria — se in futuro vengono ricollegate a un'app reale, vanno riverificate.
